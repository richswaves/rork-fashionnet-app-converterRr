...
-- This fixes the 42501 RLS error when admins or opportunity owners insert notifications
...
-- Step 2: Ensure the applicant_notifications table exists with correct structure
...
-- Step 4: Create the INSERT policy that handles ALL cases
-- Case 1: User is an admin (can insert any notification)
-- Case 2: User is an opportunity owner inserting an application notification (related_id IS NOT NULL)
-- Case 3: No restrictions for authenticated users (fallback to allow system notifications)
CREATE POLICY "Allow admins and opportunity owners to insert" 
ON applicant_notifications
FOR INSERT 
WITH CHECK (
  -- Case 1: User is an admin (can insert any notification, including profile approvals/rejections)
  EXISTS (
    SELECT 1 
    FROM user_roles
  ...
  -- This handles the case when approving/rejecting applications in the notifications screen
  (
...
-- Check 4: Test the policy - see if it allows admin inserts
-- Run this while logged in as an admin
SELECT 
  CASE 
    WHEN EXISTS (
      SELECT 1 
      ...
      WHERE user_id = auth.uid()
      AND role = 'admin'
    ) THEN 'Admin check passed - inserts should work'
    ELSE 'NOT AN ADMIN - inserts will fail'
  END as policy_check;

-- ============================================================================
-- GRANT ADMIN ROLE
-- Replace 'YOUR-USER-ID-HERE' with your actual user ID from the current session
-- ============================================================================

-- To find your user ID, run this query while logged in:
-- SELECT auth.uid();

-- Then uncomment and run this to grant yourself admin role:
...
-- Test 1: Insert a notification as an admin (should work if you're an admin)
-- INSERT INTO applicant_notifications (applicant_id, type, title, message, related_id)
-- VALUES (
--   auth.uid(),
--   'test',
--   'Test Notification',
--   'Testing notification insert as admin',
--   NULL
-- );

-- Test 2: Check if the notification was inserted
-- SELECT * FROM applicant_notifications WHERE type = 'test' ORDER BY created_at DESC LIMIT 1;
...
-- 2. Verify you're an admin: SELECT * FROM user_roles WHERE user_id = auth.uid();
-- 3. Verify the policy exists: SELECT * FROM pg_policies WHERE tablename = 'applicant_notifications';
-- 4. Try inserting a test record (see above)

-- If you see "new row violates row-level security policy", it means:
-- - You're not logged in as an admin, OR
-- - The user_roles table doesn't have an entry for you, OR
-- - The related_id check is failing (for opportunity owner inserts)
